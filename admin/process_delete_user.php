...
if (isset($_GET['user_uuid'])) {
    ...
    if ($user_uuid) {
        ...
        if ($stmt->execute([$user_uuid])) {
            ...
            header("Location: utilisateurs.php?msg=Erreur lors de la suppression de l'Utilisateur.");
            exit(); // Arrêter le script après la redirection
        }
    } else {
        header("Location: utilisateurs.php?msg=Cet Utilisateur n'existe pas ou a déjà été supprimé.");
        exit(); // Arrêter le script après la redirection
    }
} else {
    header("Location: utilisateurs.php?msg=Impossible de supprimer cet Utilisateur.");
    exit(); // Arrêter le script après la redirection
}