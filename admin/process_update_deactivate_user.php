...
if (isset($_GET["admin_uuid"])) {
    ...
    try {
        $stmt = $connexion->prepare("UPDATE admin_users SET is_active = 0 WHERE admin_uuid = :id_user");
        $stmt->bindParam(':id_user', $id_user);
        $stmt->execute();

        if ($stmt->rowCount() > 0) {
            header("Location: utilisateurs.php?message=" . urlencode("Utilisateur Inactif avec succès"));
            exit();
        } else {
